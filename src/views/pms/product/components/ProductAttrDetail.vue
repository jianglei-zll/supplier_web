<template>
  <div style="margin-top: 50px">
    ...
    >
      ...
      <el-form-item label="选择SPU：" prop="spuPlatformCode">
        <el-select
          v-model="value.spuPlatformCode"
          filterable
          remote
          reserve-keyword
          placeholder="请选择要绑定的spu"
          :remote-method="searchProductMethod"
          :loading="selectLoading"
          :disabled="isEdit"
        >
          <el-option
            v-for="item in selectSku"
            :key="item.id"
            :label="item.spuName"
            :value="item.spuPlatformCode"
          >
            <span style="float: left">{{ item.spuName }}</span>
            <span style="float: right; color: #8492a6; font-size: 13px"
              >NO.{{ item.spuPlatformCode }}</span
            >
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="商品规格：" prop="specifications">
        ...
        >
          ...
          <el-table-column label="*商家SKU编号" align="center" width="120">
            <template slot-scope="scope">
              <el-input v-model="scope.row.skuCode" :disabled="isEdit"></el-input>
            </template>
          </el-table-column>
          <el-table-column label="*平台SKU编号" align="center" width="150" v-if="isEdit">
            <template slot-scope="scope">
      ...
      <el-form-item style="text-align: center">
        ...
        <el-button
          type="primary"
          size="medium"
          @click="handleFinishCommit('productAttrForm')"
          >完成，提交商品</el-button
        >
      </el-form-item>
    </el-form>
...
import { fetchSpuList } from "@/api/product";
