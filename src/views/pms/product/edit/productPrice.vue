  ...
  <div class="modelDiv">
    ...
    >
      ...
      <el-row style="margin-top: 15px">
        ...
        <el-col :span="4" class="ft1">销售价</el-col>
        <el-col :span="4" class="ft1">划线价</el-col>
      </el-row>
      <el-row>
        <el-col :span="4">
      ...
      >
        ...
        <el-table-column label="平台SKU编码" align="center" width="150">
          <template slot-scope="scope">
            <el-input v-model="scope.row.skuPlatformCode" disabled></el-input>
          </template>
        </el-table-column>
  ...
  methods: {
          ...
          switch (length) {
            case 1:
              if (list[0].selectId == this.value.skuStockList[j].sp1) {
                this.$set(
                  this.value.skuStockList[j],
                  "enterprisePrice",
                  this.enterprisePrice
                );
    ...
    getEditAttrValues(index) {
      ...
      if (index === 0) {
        ...
        for (let i = 0; i < this.value.skuStockList.length; i++) {
          let sku = this.value.skuStockList[i];
          if (sku.sp4 != null && values.indexOf(sku.sp4) === -1) {
            values.push(sku.sp4);
          }
        }
      } else if (index === 4) {
        for (let i = 0; i < this.value.skuStockList.length; i++) {
          let sku = this.value.skuStockList[i];
          if (sku.sp5 != null && values.indexOf(sku.sp5) === -1) {
            values.push(sku.sp5);
          }
        }
      }
    ...
    handleRemoveProductAttrValue(idx, index) {
      // 添加状态可以删除
      if (!this.isEdit) {
        let value = this.selectProductAttr[idx].options[index];
        let values = this.selectProductAttr[idx].values;
        values.forEach((e, i) => {
          if (value == e) {
            this.selectProductAttr[idx].values.splice(i, 1);
          }
        });
      }
    ...
    refreshProductSkuList() {
      let oldList = this.value.skuStockList;
      let skuList = [];
      console.log();
      ...
      if (this.selectProductAttr.length === 1) {
        ...
        let values1 = this.selectProductAttr[1].values;
        let values2 = this.selectProductAttr[2].values;
        let values3 = this.selectProductAttr[3].values;
        for (let i = 0; i < values0.length; i++) {
          if (values1.length === 0) {
            skuList.push({
              sp1: values0[i],
        ...
        let values2 = this.selectProductAttr[2].values;
        let values3 = this.selectProductAttr[3].values;
        let values4 = this.selectProductAttr[4].values;
        for (let i = 0; i < values0.length; i++) {
          if (values1.length === 0) {
            skuList.push({
              sp1: values0[i],
          ...
          for (let j = 0; j < values1.length; j++) {
            ...
            for (let m = 0; m < values4.length; l++) {
              skuList.push({
                sp1: values0[i],
                sp2: values1[j],
                sp3: values2[k],
      ...
      if (this.isEdit) {
        skuList.forEach((item) => {
          for (let item1 of oldList) {
            let newValue = item.sp1 + item.sp2 + item.sp3 + item.sp4 + item.sp5;
            let oldValue =
              item1.sp1 + item1.sp2 + item1.sp3 + item1.sp4 + item1.sp5;
