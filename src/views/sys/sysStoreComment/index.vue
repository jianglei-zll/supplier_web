  ...
  <div class="app-container">
    ...
    <div class="table-container">
      ...
      >
        ...
        <el-table-column prop="memberId" label="memberId">
          <template slot-scope="scope">
            {{ scope.row.memberId }}
          </template>
        </el-table-column>
        ...
        <el-table-column prop="star" label="等级">
          <template slot-scope="scope">
            {{ scope.row.star }}
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleUpdate(index, row) {
      this.$router.push({
        path: "/sys/updateSysStoreComment",
        query: { id: row.id },
      });
    },
    handleDelete(index, row) {
      this.$confirm("是否要删除该类型", "提示", {
        ...
        deleteSysStoreComment(row.id).then((response) => {
          this.$message({
            message: "删除成功",
            type: "success",
            duration: 1000,
