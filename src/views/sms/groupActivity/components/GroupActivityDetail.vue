<template>
  ...
  <el-card class="form-container" shadow="never">
    ...
    >
        ...
        >
          ...
          <el-table-column label="绑定sku" align="center">
            <template slot-scope="scope">
              <el-button size="mini" type="text" @click="bd(scope.$index,scope.row)" v-if="!scope.row.skuId"
                >绑定sku
              </el-button>
              <div v-else>
                已绑定
      ...
      <el-form-item>
        <el-button type="primary" @click="onSubmit('helpCategoryFrom')"
          >提交</el-button
        >
        <el-button v-if="!isEdit" @click="resetForm('helpCategoryFrom')"
          >重置</el-button
        >
      </el-form-item>
    </el-form>

    <el-dialog title="选择sku" :visible.sync="bdData.show" width="50%">
      <div>
        <el-table
          :data="bdData.list"
          border
        >
          ...
          <el-table-column label="商品规格" align="center">
            <template slot-scope="scope">{{ scope.row.meno }}</template>
          </el-table-column>
           <el-table-column label="操作" align="center" width="100">
            <template slot-scope="scope">
              <el-button
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        name: [
          { required: true, message: "请输入套餐商品名称", trigger: "blur" },
          {
            min: 2,
            max: 140,
  ...
  methods: {
    // 选择sku
    selectMeno(skuId){
      this.bdData.show=false;
      console.log(skuId)
      this.helpCategory.productList[this.bdData.index].skuId=skuId
      Vue.set(this.helpCategory.productList, this.bdData.index, this.helpCategory.productList[this.bdData.index]);
    },
    // 绑定sku
    bd(index,row) {
      binding(row.id).then((response) => {
              console.log(response);
              this.bdData.list=response.data;
              this.bdData.index=index;
              this.bdData.show=true;
      });
    },
    onSubmit(formName) {
