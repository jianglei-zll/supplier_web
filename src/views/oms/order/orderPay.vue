<template> 
  <div class="app-container">
    ...
    <el-card class="operate-container" shadow="never">
      ...
      <span>已完成或者部分退款的订单，如果有拆单，取总订单 不展示商户订单</span>
    </el-card>
    <div class="table-container">
      <el-table ref="orderTable"
        ...
        <el-table-column label="支付编号" width="180" align="center">
          <template slot-scope="scope">{{scope.row.prepayId}}</template>
        </el-table-column>
        ...
        <el-table-column label="是否拆单" width="120" align="center">
                  <template slot-scope="scope">

                              <p v-if="scope.row.childList && scope.row.childList.length>0">
                                <el-button
                                  type="text"
                                  @click="handleShowVeriyEditDialog(scope.$index, scope.row)">子订单信息
                                </el-button>
                              </p>
                               <p>{{scope.row.childList | formatChildList}}</p>
                  </template>
         </el-table-column>
...
<script>
  import {orderPayList,closeOrder,deleteOrderById} from '@/api/order'
    ...
    methods: {
      ...
      handleCloseOrder(index, row){
       ...
       handleShowVeriyEditDialog(index,row){
              ...
              this.vertyProduct.list=row.childList;
            },
      ...
      getList() {
        ...
        orderPayList(this.listQuery).then(response => {
